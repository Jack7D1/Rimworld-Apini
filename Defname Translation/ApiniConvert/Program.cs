...
List<List<string>> defReplacements = new List<List<string>>{
...
List<List<string>> matchArounds = new List<List<string>>{
new List<string>{">", "<"},
new List<string>{"<", ">"},
new List<string>{"</", ">"},
new List<string>{"\"", "\""}
...
Console.WriteLine($"WARNING! This program will edit ALL xml files in this directory and all folders below. Ensure that all files in this directory are intended to be edited according to the translation table visible in Apini's root directory on the staging branch (Until ~April 30th 2022). \nThis program may make mistakes by either failing to edit names it should, or editing ones it should not, thus it is YOUR responsibility to test the edited files before deleting the originals that should be backed up. This program is tested on the Apini mod directory. By continuing you agree to take full responsibility for any data loss that takes place within this directory. \nWith that out of the way; \nProceeding will edit {files.Count} targeted xml files, continue? y/n");
{
ConsoleKeyInfo key = Console.ReadKey(true);
if(key.KeyChar != 'y' && key.KeyChar != 'Y'){Environment.Exit(75);}
}
Console.WriteLine("\n");
int totwrites = 0, editedfiles = 0;
foreach (string file in files)
{
    int writes = 0;
    string filedata = File.ReadAllText(file);
    foreach (List<string> defReplacement in defReplacements)
    {
        string oldDef = defReplacement[0], newDef = defReplacement[1];
        foreach (List<string> matchAround in matchArounds)
        {
            string prepend = matchAround[0], append = matchAround[1];
            string newfiledata = Regex.Replace(filedata, prepend + oldDef + append, prepend + newDef + append);
            if(newfiledata != filedata){writes++;}
            filedata = newfiledata;
        }
    }
    if(writes > 0)
    {
        File.WriteAllText($"{file}.tmp", filedata);
        File.Move($"{file}.tmp", file, true);
        Console.WriteLine($"{file} translated successfully with {writes} edits.");
        totwrites += writes;
        editedfiles++;
    }
    else
    {
        Console.WriteLine($"{file} had no matching defs and was not changed.");
    }
    
}
Console.WriteLine($"Done! A total of {editedfiles} files were edited, making a total of {totwrites} names replaced.");
Console.Beep();
